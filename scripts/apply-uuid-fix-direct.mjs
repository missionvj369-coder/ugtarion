/**
 * Direct UUID Fix - Applies the fix using Supabase Management API
 * Run: node scripts/apply-uuid-fix-direct.mjs
 */
...
async function applyFix() {
  console.log('🔧 Applying UUID Fix via Supabase Management API...\n');

  try {
    const response = await fetch(`${SUPABASE_URL}/rest/v1/rpc/pg_catalog.pg_execute_service_sql`, {
      method: 'POST',
      headers: {
    ...
    if (response.ok) {
      ...
      console.log('⚠️ API approach failed:', response.status);
      console.log('Error:', errorText);
      
      // Try direct management API
      console.log('\n🔄 Trying Management API...');
      const mgmtResponse = await fetch('https://api.supabase.com/v1/projects/mgrdamgdpnbtxgxdxwbs/database/query', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${SUPABASE_SERVICE_KEY}`,
      ...
      if (mgmtResponse.ok) {
        console.log('✅ Fix applied via Management API!');
      } else {
        const mgmtError = await mgmtResponse.text();
        console.log('❌ Management API also failed:', mgmtResponse.status);
        console.log('Error:', mgmtError);
      }
    }
  } catch (e) {
  ...
  console.log('\n📋 If fix failed, please run uuid-fix-temp.sql manually in Supabase SQL Editor');
