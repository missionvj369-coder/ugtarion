...
async function test() {
  // Check the register_user_with_password function
  const response = await fetch('https://api.supabase.com/v1/projects/mgrdamgdpnbtxgxdxwbs/database/query', {
    ...
    body: JSON.stringify({
      query: "SELECT prosrc FROM pg_proc WHERE proname = 'register_user_with_password';"
    })
  });
  
  const result = await response.json();
  console.log('register_user_with_password function:');
  console.log('Status:', response.status);
  console.log('Result:', JSON.stringify(result, null, 2));
  
  // Also check login_user_atomic
  const response2 = await fetch('https://api.supabase.com/v1/projects/mgrdamgdpnbtxgxdxwbs/database/query', {
    ...
    body: JSON.stringify({
      query: "SELECT prosrc FROM pg_proc WHERE proname = 'login_user_atomic';"
    })
  });
  
  const result2 = await response2.json();
  console.log('\nlogin_user_atomic function:');
